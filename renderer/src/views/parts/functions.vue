<template>
  <div>
              ...
              <n-button
                ...
                @click="
                  $router.push({
                    ...
                    params: { app: props.data.slug, function: item.slug },
                  })
                "
              >
                <template #icon>
    ...
    <div class="flex justify-between mt-4">
      <n-button type="primary" size="small" @click="$router.push({ name: 'console.apps.functions.create' })">
        <template #icon>
...
async function fetch() {
  try {
    ...
    rawData.value = (await http.get(`/api/apps/${props.data.slug}/functions`)).data
  } catch (e: any) {
  ...
  $dialog.warning({
    ...
    onPositiveClick: async () => {
      try {
        ...
        await http.delete(`/api/apps/${props.data.slug}/functions/${item.slug}`)
        await fetch()

        $message.success("Successfully deleted the function.")
      } catch (e: any) {
        $message.error(`Something went wrong... ${e}`)
      } finally {
        requesting.value = false
